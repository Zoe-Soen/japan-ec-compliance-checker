...
> 文档版本：v0.2
>
> 上一版：`docs/MVP技术方案.md`（v0.1，Git commit `f59b72b`）
>
> 状态：待 Zoe 确认

## 1. 结论摘要

MVP 采用 **Web SaaS + 独立扫描 Worker**，不采用 Electron 作为主应用。技术形态与收费方式分开决策：系统保持在线服务形态，验证期优先支持按次额度包，不直接假设用户接受无限次月费订阅。
...
| 用量计量 | 扫描用量台账 | 记录任务类型和资源消耗，为按次额度及后续套餐提供依据 |
| OCR | 可替换的 OCR 适配层 | 先验证本地日文 OCR，准确率不足时再接入云端 OCR |
| 部署 | 可按需运行的 Docker 容器，日本区域优先 | 隔离扫描环境，并减少试用期空闲资源成本 |

Electron 保留为未来的“本地扫描助手”候选，只在必须使用客户本机登录状态、本机 IP、证书或本地文件时考虑。

商业变现逻辑见 [`MVP商业变现逻辑方案.md`](MVP商业变现逻辑方案.md)。本文件只规定收费方式对系统提出的技术约束，不在代码中提前固化尚未验证的套餐和价格。
...
- 第一批用户预期是每年反复检查多个网站的代理运营人员。
- 同一网站的新商品上线会产生范围更小、但可能重复发生的检查需求。
...
- “使用 Web SaaS”和“采用月费订阅”不是同一个决定；系统可以通过按次额度包收费。
- 扫描成本随页面数、运行时间、OCR 和文件保存量变化，必须记录单次任务资源用量。
...
- 额度包、代理套餐和超额使用的具体计量规则。
- 新商品快速检查的规则范围和额度消耗。

### 2.4 已确认的商业约束

- MVP 技术形态仍为 Web SaaS。
- 验证期不实现无限次套餐，避免重度使用与固定低月费不匹配。
- 正式支付系统不是最小真实扫描的前置条件；首批试用可以人工发放和扣减额度。
- 技术层只记录可审计的用量，不在规则引擎中混入价格逻辑。
- 买断式私有部署如未来存在，应作为独立交付方案，并另行约定规则更新和技术支持期限。
...
系统从一开始预留三种任务类型，但当前 MVP 只实现第一种：

| 任务类型 | 技术标识 | MVP 状态 | 说明 |
|---|---|---|---|
| 网站完整检查 | `site_full` | 实现 | 按当前 30 条规则检查公开网站和补充截图 |
| 新商品快速检查 | `product_quick` | 只预留 | 面向单个或少量商品页面，规则范围待验证 |
| 整改复查 | `recheck` | 只预留 | 基于上次结果重新检查，并识别已改善和仍存在的问题 |

任务类型只决定检查范围和用量记录，不允许绕过安全策略。化妆品、保健品、食品等专项商品规则仍不属于 MVP。

## 6. 核心任务流程

1. 用户提交 URL、项目名称和业务问卷。
2. Web 应用完成 URL 格式检查，并创建带有任务类型的扫描任务。
3. Worker 原子领取任务，记录规则版本和开始时间。
4. Worker 校验 DNS、IP 和每次跳转，阻止内网及危险目标。
5. Playwright 在隔离环境中发现和抓取页面。
6. 提取器将页面内容转换为结构化事实。
7. 规则引擎执行 R01～R30，并保存证据与判断原因。
8. Worker 写入页面数、执行秒数、OCR 次数和文件量等资源用量。
9. Web 应用读取进度，任务完成后展示在线报告并记录额度事件。
10. 用户可重新检查；新结果作为独立版本保存，不覆盖旧结果。
...
| UsageLedger | 额度增加／扣减、任务类型、关联扫描和操作来源 |
...
- 为降低成本，默认不永久保存完整页面副本；结构化证据和报告可以保留，原始截图按保存期限自动删除。
...
- Web 和数据库可以先使用低规格或有免费额度的托管服务，但不得依赖无法满足安全要求的免费方案。
- 扫描 Worker 优先采用按任务启动或最低规格运行，不为尚不存在的并发长期预留资源。
- 原始截图和页面文件设置自动删除策略，避免无上限累积。
- 每月记录固定成本、扫描计算、OCR、存储和带宽，达到付费试用前不引入非必要服务。
- 云服务免费额度可能变化，正式上线前必须重新核对价格和数据处理地区。
...
- **用量测试**：任务只扣减一次额度；失败、重试、取消和人工调整均有明确账目。
...
- 每次扫描都有任务类型、资源用量和唯一额度事件，失败重试不会重复扣减。
...
- 建立 web、worker、rules、shared、report 和 usage 工作区；
- 接入 PostgreSQL 和对象存储；
- 实现创建任务、进度和模拟结果链路。
- 建立 `site_full`、`product_quick`、`recheck` 类型和最小用量台账；只开放 `site_full`。
...
- 记录每次扫描的真实成本、任务频率和复查比例；
- 优先修正规则和提取准确性，不扩展国家或专项商品法规。

完成标志：试用用户能够独立完成检查，并认为至少 80% 的问题可以直接进入整改。

`product_quick` 和 `recheck` 的具体规则在试用数据证明存在稳定需求后进入 MVP+1，不提前扩大当前 30 条规则范围。
...
| 验证期收费 | Web SaaS + 按次额度包，暂不做无限次订阅 | 已确认方向 |
| 任务类型 | `site_full` 实现；`product_quick`、`recheck` 预留 | 第一版基线 |
| 支付系统 | 首轮可人工管理额度，正式支付接入时点未定 | 待确认 |
...
11. [Cloudflare R2 定价](https://developers.cloudflare.com/r2/pricing/)
12. [Supabase 定价](https://supabase.com/pricing)

## 18. 配套文档

- [`MVP 产品设计文档`](MVP产品设计文档.md)
- [`MVP 商业变现逻辑方案`](MVP商业变现逻辑方案.md)
